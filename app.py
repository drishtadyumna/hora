...
def fetch_coordinates(place: str):
    ...
    try:
        r = requests.get(GEOCODE_API_URL, params={"q":place}, timeout=5)
        r.raise_for_status()
        data = r.json()
...
with st.form("birth"):
    ...
    with c2:
        ...
        ayn = st.selectbox("Ayanamsha", ["lahiri","sayana"],
                           index=["lahiri","sayana"]
...
if st.button("Fetch Astrological Data", type="primary"):
    ...
    if missing:
        st.error("Birth details incomplete. Missing: " + ", ".join(missing))
    else:
        ...
        if not chosen:
            ...
            for idx,chart in enumerate(chosen):
                ...
                res = fetch_astro_data(api_key,
                                       CHART_ENDPOINTS[chart],
                                       payload)
                results[chart] = res
...
if st.session_state.get("results"):
    ...
    name = "".join(c for c in st.session_state.birth_info.get("name","chart")
    ...
    with c1:
        st.download_button("Download Raw JSON",
                           raw, file_name=f"{base}_raw.json")
    with c2:
        st.download_button("Download Readable TXT",
                           txt, file_name=f"{base}_readable.txt")
...
if st.button("Clear All & Start Over"):
    # Explicitly clear all keys except user_api_key
